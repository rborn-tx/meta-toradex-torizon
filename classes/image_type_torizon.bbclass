...
# The "TCB signing files" tarball is built and deployed by the tcb-signing-files
# recipe; the definitions shared by both sides live next to it.
...
python add_signing_files_to_tezi_artifacts() {
    ...
    # Ordering matters and nothing else states it: rootfs_tezi_run_json() *sets*
    # TEZI_ARTIFACTS rather than appending to it, so anything added before it runs
    # is discarded without a trace. This function must therefore stay after it in
    # TEZI_IMAGE_TEZIIMG_PREFUNCS. An empty TEZI_ARTIFACTS here means that
    # ordering has been broken -- say so instead of silently shipping a Tezi image
    # without the tarball.
